...
const view = (elements, validate, feed, state, urlFeed) => {
  ...
  posts.addEventListener('click', (e) => {
    ...
    const currentFirst = currentList.firstChild
    const hrefPost = currentFirst.getAttribute('href')

    currentFirst.classList.remove('fw-bold')
    currentFirst.classList.add('fw-normal', 'link-secondary')

    state.feeds.forEach((feed) => {
      const post = feed.items.filter(item => item.link === hrefPost)
      modalOpen(post)
    })
  })
}
